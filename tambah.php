<?php
include 'config.php';

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $nama = $_POST['nama_lomba'];
    $kategori = $_POST['id_kategori'];
    $jadwal = $_POST['id_jadwal'];
    $hadiah = $_POST['id_hadiah'];

    $sql = "INSERT INTO lomba (nama_lomba, id_kategori, id_jadwal, id_hadiah)
            VALUES ('$nama', $kategori, $jadwal, $hadiah)";
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Data berhasil ditambahkan!'); window.location='index.php';</script>";
    } else {
        echo "<script>alert('Gagal menambahkan data!');</script>";
    }
}

$kategori = mysqli_query($conn, "SELECT * FROM kategori");
$jadwal = mysqli_query($conn, "SELECT * FROM jadwal_lomba");
$hadiah = mysqli_query($conn, "SELECT * FROM hadiah");
?>

<!DOCTYPE html>
<html>
<head>
    <title>Tambah Lomba</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="container mt-4">

<h2>Tambah Data Lomba</h2>
<form method="POST">
    <div class="mb-3">
        <label class="form-label">Nama Lomba:</label>
        <input type="text" name="nama_lomba" class="form-control" required>
    </div>

    <div class="mb-3">
        <label class="form-label">Kategori:</label>
        <select name="id_kategori" class="form-select">
            <?php while($k = mysqli_fetch_assoc($kategori)) echo "<option value='{$k['id']}'>{$k['nama_kategori']}</option>"; ?>
        </select>
    </div>

    <div class="mb-3">
        <label class="form-label">Jadwal:</label>
        <select name="id_jadwal" class="form-select">
            <?php while($j = mysqli_fetch_assoc($jadwal)) echo "<option value='{$j['id']}'>{$j['tanggal_mulai']} - {$j['tanggal_selesai']}</option>"; ?>
        </select>
    </div>

    <div class="mb-3">
        <label class="form-label">Hadiah:</label>
        <select name="id_hadiah" class="form-select">
            <?php while($h = mysqli_fetch_assoc($hadiah)) echo "<option value='{$h['id']}'>{$h['nama_hadiah']}</option>"; ?>
        </select>
    </div>

    <button type="submit" class="btn btn-success">Simpan</button>
    <a href="index.php" class="btn btn-secondary">Kembali</a>
</form>

</body>
</html>
